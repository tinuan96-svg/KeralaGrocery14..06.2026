...
function toProduct(row: ProductRow, catMap: Record<string, LookupRow>, brdMap: Record<string, LookupRow>): BannerProduct {
  ...
  return {
    ...
    stock: row.stock ?? 0,
    backorder_enabled: row.backorder_enabled ?? false,
    discount,
    cat_id: cat?.id ?? null,
...
const SELECT = 'id,name,slug,price,original_price,discount_percentage,image_url,image_main,enhanced_image_url,image_medium,is_featured,is_bestseller,is_new_arrival,sold_count,stock,backorder_enabled,category_id,brand_id,created_at';
