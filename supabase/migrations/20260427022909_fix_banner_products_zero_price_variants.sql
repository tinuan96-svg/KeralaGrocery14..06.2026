/*
  # Fix 0 price for WooCommerce variable products in dynamic banner products

  ## Problem
  get_dynamic_banner_products had the same issue: `p.price` as the final
  fallback in the COALESCE chain, which is 0 for variable products.
  Also `p.original_price` fallback can be 0.

  ## Fix
  Wrap final p.price fallbacks with NULLIF(..., 0).
  Add a WHERE filter to exclude products with no resolvable price from banners.
*/
...
WITH eligible AS (
  SELECT
    ...
    -- Prefer processed clean image first, then store override, then raw
    COALESCE(
      NULLIF(p.image_main, ''),
  ...
  WHERE p.is_active  = true
    ...
    -- Exclude variable products with no resolvable price
    AND COALESCE(
      NULLIF(sp.price_override, 0),
      NULLIF(kg.adjusted_price, 0),
      NULLIF(kg.price, 0),
