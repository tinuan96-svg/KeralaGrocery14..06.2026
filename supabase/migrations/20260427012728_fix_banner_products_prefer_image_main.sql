/*
  # Fix get_dynamic_banner_products to prefer image_main over store override

  The previous COALESCE order was:
    sp.image_override → p.image_main → p.image_url

  This meant old store_products.image_override values (raw bucket URLs)
  blocked the newly processed clean images from showing.

  Fixed order:
    p.image_main → sp.image_override → p.image_url

  Clean processed images now always take precedence.
*/
